    ...
    <div class="card mb-5 mb-xl-10">
        ...
        <div class="card-header border-0 cursor-pointer" role="button" data-bs-toggle="collapse"
            ...
            <div class="card-title m-0">
                <h3 class="fw-bolder m-0"> تعديل إختبار >> {{$test->test}} </h3>
            </div>
            <!--end::Card title-->
        </div>
        <!--begin::Card header-->
        ...
        <div id="kt_account_profile_details" class="collapse show">
            <!-- Start Form -->
            <form action="{{ route('tests.update', 'test')}}" method="POST" id="create">
                @csrf
                @method('PATCH')

                <input type="hidden" name="id" value="{{$test->id}}" required/>
                ...
                <div class="card-body border-top p-9">
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-10 fv-row">
                            <input type="text" name="test" class="form-control form-control-lg form-control-solid"
                                   placeholder="ادخل الإختبار" value="{{$test->test}}" required/>
                            @error('test')
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-10 fv-row">
                            <input type="text" name="passing" class="form-control form-control-lg form-control-solid"
                                   ...
                                   placeholder="درجة النجاح" value="{{$test->passing}}" required/>
                            @error('passing')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="row mb-6">
                        <label class="col-lg-2 col-form-label required fw-bold fs-6">نمط الاختبار</label>
                        <div class="col-lg-10 fv-row">
                            <select class="form-select form-select-solid" data-control="select2" data-hide-search="true"
                                    data-placeholder="Select a Team Member" name="pattern" required>
                                @foreach($patterns as $pattern)
                                    <option value="{{$pattern->id}}" {{$pattern->id == $test->pattern_id ? 'selected' : ''}}>{{$pattern->title}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
